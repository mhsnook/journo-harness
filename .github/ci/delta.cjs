...
const parsers = {
	...
	// unix format, emitted by oxlint -f unix and most other linters:
	// src/foo.ts:12:5: message [rule]
	unix: (raw) => {
...
function countSummary(d, noun, { showTrend = false } = {}) {
	...
	const change =
		d.added.length || d.resolved.length
			...
			: ' (no change)'
	const before = `**Before:** ${d.base} ${noun}`
	const after = `**After:** ${d.head} ${noun}`
	return `${prefix}${before} → ${after}${change}${movedNote(d.moved)}`
}
...
/** Group file paths by extension: 80 .css files read differently from 80 .tsx. */
...
function sizeTable(headRaw, headGz, baseRaw, baseGz) {
	const raw = `${formatBytes(baseRaw)} | ${formatBytes(headRaw)}`
	const gz = `${formatBytes(baseGz)} | ${formatBytes(headGz)}`
	return [
		'|         | Base | Head | Δ |',
		'|---------|------|------|---|',
		`| Raw | ${raw} | ${deltaLabel(headRaw, baseRaw)} |`,
		`| Gzipped | ${gz} | ${deltaLabel(headGz, baseGz)} |`,
	].join('\n')
}
...
function selftest() {
	...
	const any = 'no-explicit-any'

	// A pure line shift is paired, not double-counted.
	const shifted = differential([`a.ts:10:5: ${any}`], [`a.ts:14:5: ${any}`], p)
	assert.equal(shifted.added.length, 0, 'a shift within tolerance is not new')
	assert.equal(shifted.resolved.length, 0, 'a shift within tolerance is not resolved')
	assert.equal(shifted.moved.length, 1)

	// Beyond the tolerance it is a genuine change.
	const far = differential([`a.ts:10:5: ${any}`], [`a.ts:99:5: ${any}`], p)
	assert.equal(far.added.length, 1, 'a shift beyond tolerance counts as new')
	assert.equal(far.resolved.length, 1)

	// A changed column is a real new issue even when the line barely moved.
	const recolumned = differential([`a.ts:10:5: ${any}`], [`a.ts:11:9: ${any}`], p)
	assert.equal(recolumned.added.length, 1, 'a changed column counts as new')
	...
	// oxlint's unix format parses into the same shape.
	const u = parsers.unix('src/client/App.tsx:3:1: Unused import [no-unused-vars]')
	assert.equal(u.file, 'src/client/App.tsx')
	assert.equal(u.line, 3)
	...
	console.log('delta.cjs self-test passed (8 cases)')
}
