...
// Turn two directories of collected static-check output into markdown fragments
// plus a gate sidecar. Section order comes from the numeric filename prefix, so
// fragments render consistently whatever order the jobs finish in.
...
const CHECKS = [
	{ fragment: '10-typecheck', check: 'typecheck', title: 'Type errors' },
	{ fragment: '20-lint', check: 'lint', title: 'Lint' },
	{ fragment: '30-format', check: 'format', title: 'Formatter drift' },
]
...
module.exports = function render({ head, base, out }) {
	...
	// A whole tree's output is absent when its job died before uploading. Every
	// list would then read as empty, and an empty list against an empty list is
	// "no change" — a crashed job would report as a clean PR.
	if (!fs.existsSync(head) || !fs.existsSync(base)) {
		const which = !fs.existsSync(head) ? 'The PR' : 'The base branch'
		for (const { fragment, check, title } of CHECKS) {
			write(
				fragment,
				`#### ${title}\n\n⚠️ ${which} job produced no measurement, so there is nothing to compare. Check the job log.`,
				{ check, missing: true },
			)
		}
		return
	...
	write(
		...
		[
			'#### Formatter drift',
			'',
			'Files `oxfmt` would still rewrite — debt to drive toward **0** by reformatting legacy files as you touch them.',
			'',
