# Phase 1 storage: the Plan in Article Agent state, the House in party-db over D1
...
- **One Article Agent per Article**, holding that Article's Chat, its Plan, its
  Notes, and its Offers. Arrives in 1a.
...
So 1a needs no store beyond the Article Agent, no second connection, and no write path of
its own.
...
**Article Agent state for everything, forever.** Rejected because Article Agent state is a whole-blob
...
- **Only the Plan lives in Article Agent state.** It is a whole-blob replace, so it can
  have exactly one owner, and that owner is the writer. The Notes, the Rounds, and the
  Offers are guide-written and go in the Article Agent's SQLite tables, read by the client
  over `@callable` RPC on the socket it already holds. Settled in #11.
